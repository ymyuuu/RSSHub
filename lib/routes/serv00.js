...
module.exports = async (ctx) => {
    ...
    if (match) {
        ...
        // 设置 RSS 数据
        ctx.state.data = {
            title: 'Serv00 - Account Number Monitoring',
            link: url,
            item: [{
                title: `Serv00 Account Number: ${currentAccounts} / ${maxAccounts}`,
                description: `
                    <p>Current account number is <strong>${currentAccounts}</strong>.</p>
                    <p>The maximum account number is <strong>${maxAccounts}</strong>.</p>
                    <p>Data update time: ${beijingTime}</p>
                `,
                link: url,
